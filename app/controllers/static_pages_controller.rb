class StaticPagesController < ApplicationController
  ...
  def home
    ...
    @top_quotes = MPQ.joins(:quotes).includes(:quotes).order(score: :desc).limit(5).page(1).per(20)
    @recent_quotes = MPQ.joins(:quotes).includes(:quotes).order(created_at: :desc).limit(5).page(1).per(20)
